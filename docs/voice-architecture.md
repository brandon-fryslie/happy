...
They have to agree because the conversation token is a JWT signed by one provider's LiveKit keys. Present it to a different SFU and nothing errors — the client joins a room the agent is not in and the user hears silence. `voiceProvider.ts` on each side keeps its half in one place: `meteredConvaiApi()` on the server, and `voiceMint()` plus `livekitUrlFor()` in the app.

`RealtimeSession.ts` is the app's one caller of those: it reads the settings once, picks the mint, fetches a token against it, and threads the matching `livekitUrl` into `VoiceSessionConfig`. The web bridge takes that value straight through to `startSession` and never imports `voiceProvider` at all. On web, do not reach for `origin` instead — that configures the WebSocket signaling path, which Happy never takes because it always supplies a `conversationToken`. Setting it moves nothing and looks like it worked.

The native bridge is the exception, because the React Native SDK fixes its SFU as `serverUrl` when the `useConversation` hook renders and applies it through its own effect — there is no way to hand it a URL at `startSession` time. So the native bridge calls `voiceProvider` itself, from live settings, and records what that render gave the SDK. `startSession` then compares that against the `livekitUrl` the mint produced and throws if they differ. They differ only when the voice settings changed during the token fetch, which makes the whole call stale — wrong agent and wrong credentials, not just the wrong SFU — so the abort is the correct outcome, and it is loud instead of silent.
