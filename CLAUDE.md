...
### Voice (ElevenLabs)
The mobile app integrates ElevenLabs ConvAI for hands-free agent control (mic button in `SessionView.tsx`). `RealtimeSession.ts` is the lifecycle owner; `RealtimeVoiceSession.tsx` (native) and `.web.tsx` (web) are the SDK bridges. The voice agent invokes the client tools declared in `realtimeClientTools.ts` — `sendMessageToSession` and `processPermissionRequest` — whose names and parameter schemas live in `voiceToolContract.ts`, the one declaration the SDK registration and the BYO setup guide are both derived from. The agent addresses sessions and permission requests by id, reading both out of the context Happy injects (see `docs/voice-architecture.md`).
