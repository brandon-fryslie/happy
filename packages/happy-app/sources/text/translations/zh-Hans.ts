...
export const zhHans: TranslationStructure = {
    ...
    settingsVoice: {
        ...
        byoTitle: '使用自己的代理',
        byoDescription: '使用您自己的 ElevenLabs 代理代替 Happy 默认代理。无需订阅 — 直接使用您自己的 ElevenLabs 账户连接。您的代理必须定义下方“代理提示指南”中列出的客户端工具，并通过 {{initialConversationContext}} 动态变量接收会话上下文。',
        ...
        promptGuideDescription: ({ toolSignatures }: { toolSignatures: string }) => `您的 ElevenLabs 代理需要以下客户端工具，名称和参数必须完全一致：\n\n${toolSignatures}\n\n还需要动态变量 {{initialConversationContext}}，它在启动时接收会话目录和历史记录。\n\nHappy 会在注入的上下文中标明每个会话和每个权限请求：会话 id 出现在初始目录以及每次焦点或状态更新中，权限请求则包裹在 <request_id> 标签内。您的代理应原样回传这些 id — 绝不能自行编造。\n\n代理充当用户和编码代理之间的语音桥梁。它应该简洁，仅在被呼叫时回应，并在编码代理完成工作时进行报告。`,
    ...
    settingsTts: {
        ...
        enabledTitle: '会话朗读',
        enabledFooter: '使用您自己的 LLM 总结代理消息,并通过 ElevenLabs 朗读出来。一切都保留在您的设备上 — 密钥和消息文本永不发送到 Happy 服务器。',
        enableLabel: '启用',
        enableSubtitleOn: '会话中显示朗读控制栏',
        enableSubtitleOff: '在会话中隐藏',
        autoSpeakTitle: '自动朗读',
        autoSpeakFooter: '免提模式在应用位于后台或屏幕锁定时继续朗读，并逐条朗读每个回复，中间不停顿。',
        autoSpeakOffLabel: '关闭',
        autoSpeakOffSubtitle: '仅在点击播放时朗读',
        autoSpeakForegroundLabel: '应用打开时',
        autoSpeakForegroundSubtitle: '应用打开时朗读代理的新消息，回复之间间隔 30 秒',
        autoSpeakHandsFreeLabel: '免提',
        autoSpeakHandsFreeSubtitle: '在屏幕关闭或应用位于后台时继续朗读，回复之间没有间隔',
        ...
        elevenLabsFooter: '语音合成。设置 TTS 专用密钥时,优先级高于 BYO 语音密钥。',
        voiceIdLabel: '语音 ID',
        voiceIdSubtitleNotSet: '默认语音 (Rachel)',
        voiceIdPrompt: '输入 ElevenLabs 语音 ID。留空使用默认值。',
        voiceIdPlaceholder: '21m00Tcm4TlvDq8ikWAM',
        elevenLabsKeyLabel: 'API Key',
        elevenLabsKeySubtitleNotSet: '未设置时回退到 BYO 语音密钥',
        elevenLabsKeySubtitleSet: '已配置 (已隐藏)',
        elevenLabsKeyPrompt: '输入您的 ElevenLabs API key。留空则回退到 BYO 语音密钥。',
        elevenLabsKeyPlaceholder: 'sk_…',
        llmTitle: '总结 (兼容 OpenAI)',
        llmFooter: '任何兼容 OpenAI 的端点:OpenAI、OpenRouter、Ollama、vLLM 等。对于本地 Ollama,请使用 Tailscale 或 LAN 主机名 (例如 http://ollama.local:11434/v1) — localhost 在手机上无法使用。',
        llmBaseUrlLabel: 'Base URL',
        llmBaseUrlSubtitleNotSet: '未配置',
        llmBaseUrlPrompt: '输入兼容 OpenAI 的 base URL。',
        llmBaseUrlPlaceholder: 'https://api.openai.com/v1',
        llmKeyLabel: 'API Key',
        llmKeySubtitleNotSet: '未配置 (本地 Ollama 可留空)',
        llmKeySubtitleSet: '已配置 (已隐藏)',
        llmKeyPrompt: '输入 LLM 的 API key。本地 Ollama 可以留空。',
        llmKeyPlaceholder: 'sk-…',
        llmModelLabel: '模型',
        llmModelSubtitleNotSet: '未配置',
        llmModelPrompt: '输入模型名称。',
        llmModelPlaceholder: 'gpt-4o-mini',
        privacyTitle: '隐私',
        privacyText: 'API keys 在本设备上本地存储。消息文本将直接发送到您配置的 LLM 和 ElevenLabs 端点 — 永不发送到 Happy 服务器。',
    },
    ...
    sessionTts: {
        ...
        speak: '朗读',
        stop: '停止',
        continueOption: '从上次继续',
        fromLastUser: '从您最后的消息开始',
        restart: '从头开始',
        loading: '正在总结…',
        playing: '正在朗读…',
        nothingNew: '没有新内容可读',
        configurePrompt: '在设置中配置 TTS',
    },
    ...
    imageUpload: {
        ...
        cliTooOldTitle: '请更新该机器上的 Happy',
        cliTooOldMessage: ({ requiredVersion }: { requiredVersion: string }) =>
            `运行此会话的机器需要 happy CLI ${requiredVersion} 或更高版本才能接收图片。仅发送了文本。`,
    },
