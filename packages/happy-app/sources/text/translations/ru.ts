...
export const ru: TranslationStructure = {
    ...
    settings: {
        ...
        tts: 'Озвучивание сессий',
        ttsSubtitle: 'Сжимать и зачитывать сообщения агента вслух',
    ...
    errors: {
        ...
        voiceByoCredentialsMissing: 'Direct Connection is on, but no ElevenLabs Agent ID and API Key are configured. Add them in Voice settings, or turn Direct Connection off to use the Happy default agent.',
    ...
    settingsVoice: {
        ...
        byoDescription: 'Используйте собственного агента ElevenLabs вместо стандартного Happy. Подписка не требуется — подключайтесь напрямую через свой аккаунт ElevenLabs. Ваш агент должен определить клиентские инструменты, перечисленные в «Руководстве по промпту агента» ниже. Контекст сессии передаётся через динамическую переменную {{initialConversationContext}}.',
        ...
        customElevenLabsApiKey: 'ElevenLabs API Key',
        customElevenLabsApiKeyNotSet: 'Not configured',
        customElevenLabsApiKeySet: 'Configured (hidden)',
        customElevenLabsApiKeyDescription: 'Required for private agents. Your key is sent to the Happy server only to mint a short-lived conversation token from ElevenLabs; it is never stored server-side.',
        customElevenLabsApiKeyPlaceholder: 'sk_…',
        ...
        promptGuideDescription: ({ toolSignatures }: { toolSignatures: string }) => `Вашему агенту ElevenLabs нужны эти клиентские инструменты — ровно с такими именами и параметрами:\n\n${toolSignatures}\n\nТакже нужна динамическая переменная {{initialConversationContext}}, которая при запуске получает список сессий и историю.\n\nHappy называет каждую сессию и каждый запрос разрешения в передаваемом контексте: id сессий появляются в начальном списке и в каждом обновлении фокуса или статуса, а запрос разрешения приходит в тегах <request_id>. Агент возвращает эти id как есть — он никогда не должен их выдумывать.\n\nАгент выступает голосовым мостом между пользователем и агентами кодирования. Он должен быть кратким, отвечать только при обращении и сообщать, когда агент кодирования завершает работу.`,
    ...
    settingsTts: {
        // Speak Sessions (TTS summarize-and-speak) settings screen
        enabledTitle: 'Озвучивание сессий',
        enabledFooter: 'Сжимайте сообщения агента с помощью собственного LLM и зачитывайте их вслух через ElevenLabs. Всё остаётся на вашем устройстве — ключи и текст сообщений никогда не попадают на серверы Happy.',
        enableLabel: 'Включить',
        enableSubtitleOn: 'Панель управления озвучиванием отображается в сессиях',
        enableSubtitleOff: 'Скрыта в сессиях',
        autoSpeakTitle: 'Автоозвучивание',
        autoSpeakFooter: 'В режиме «свободные руки» озвучивание продолжается, когда приложение свёрнуто или экран заблокирован, и читается каждый ответ без пауз между ними.',
        autoSpeakOffLabel: 'Выключено',
        autoSpeakOffSubtitle: 'Озвучивать только по нажатию кнопки воспроизведения',
        autoSpeakForegroundLabel: 'Когда приложение открыто',
        autoSpeakForegroundSubtitle: 'Зачитывать новые сообщения агента, пока приложение открыто, с паузой 30 секунд между ответами',
        autoSpeakHandsFreeLabel: 'Свободные руки',
        autoSpeakHandsFreeSubtitle: 'Продолжать чтение при выключенном экране или свёрнутом приложении, без пауз между ответами',
        providerTitle: 'Провайдер озвучивания',
        providerFooter: 'Сервис, который превращает сжатый текст в речь. У каждого провайдера свой голос и свой ключ, поэтому переключение между ними сохраняет оба набора настроек.',
        providerElevenLabsLabel: 'ElevenLabs',
        providerElevenLabsSubtitle: 'Самые естественные голоса. Нужен отдельный аккаунт ElevenLabs.',
        providerOpenAiLabel: 'OpenAI',
        providerOpenAiSubtitle: 'Использует ваш ключ OpenAI, в том числе ключ для сжатия',
        openAiTitle: 'Речь OpenAI',
        openAiFooter: 'Используется, когда выбран провайдер озвучивания OpenAI. Если ключ не задан, используется ключ сжатия ниже.',
        openAiVoiceLabel: 'Голос',
        openAiVoiceSubtitleNotSet: 'Голос по умолчанию (Alloy)',
        openAiVoicePrompt: 'Введите название голоса OpenAI: alloy, echo, fable, onyx, nova или shimmer. Оставьте пустым для голоса по умолчанию.',
        openAiVoicePlaceholder: 'alloy',
        openAiKeyLabel: 'API Key',
        openAiKeySubtitleNotSet: 'Если не задан, используется ключ сжатия',
        openAiKeySubtitleSet: 'Настроен (скрыт)',
        openAiKeyPrompt: 'Введите ваш API key OpenAI. Оставьте пустым, чтобы использовать ключ сжатия.',
        openAiKeyPlaceholder: 'sk-…',
        elevenLabsTitle: 'ElevenLabs',
        elevenLabsFooter: 'Синтез речи. Если задан, специальный ключ TTS имеет приоритет над ключом голоса BYO.',
        voiceIdLabel: 'ID голоса',
        voiceIdSubtitleNotSet: 'Голос по умолчанию (Rachel)',
        voiceIdPrompt: 'Введите ID голоса ElevenLabs. Оставьте пустым для голоса по умолчанию.',
        voiceIdPlaceholder: '21m00Tcm4TlvDq8ikWAM',
        elevenLabsKeyLabel: 'API Key',
        elevenLabsKeySubtitleNotSet: 'Если не задан, используется ключ голоса BYO',
        elevenLabsKeySubtitleSet: 'Настроен (скрыт)',
        elevenLabsKeyPrompt: 'Введите ваш API key ElevenLabs. Оставьте пустым, чтобы использовать ключ голоса BYO.',
        elevenLabsKeyPlaceholder: 'sk_…',
        llmTitle: 'Сжатие (совместимое с OpenAI)',
        llmFooter: 'Любой совместимый с OpenAI endpoint: OpenAI, OpenRouter, Ollama, vLLM и т. д. Для локального Ollama используйте имя хоста Tailscale или LAN (например, http://ollama.local:11434/v1) — localhost не работает с телефона.',
        llmBaseUrlLabel: 'Базовый URL',
        llmBaseUrlSubtitleNotSet: 'Не настроено',
        llmBaseUrlPrompt: 'Введите базовый URL, совместимый с OpenAI.',
        llmBaseUrlPlaceholder: 'https://api.openai.com/v1',
        llmKeyLabel: 'API Key',
        llmKeySubtitleNotSet: 'Не настроено (оставьте пустым для локального Ollama)',
        llmKeySubtitleSet: 'Настроен (скрыт)',
        llmKeyPrompt: 'Введите API key для LLM. Может быть пустым для локального Ollama.',
        llmKeyPlaceholder: 'sk-…',
        llmModelLabel: 'Модель',
        llmModelSubtitleNotSet: 'Не настроено',
        llmModelPrompt: 'Введите название модели.',
        llmModelPlaceholder: 'gpt-4o-mini',
        privacyTitle: 'Конфиденциальность',
        privacyText: 'API keys хранятся локально на этом устройстве. Текст сообщений отправляется напрямую к настроенным вами endpoint LLM и ElevenLabs — никогда на серверы Happy.',
    },

    sessionTts: {
        // Speak Sessions in-session control bar
        speak: 'Озвучить',
        stop: 'Стоп',
        continueOption: 'Продолжить с последнего',
        fromLastUser: 'С вашего последнего сообщения',
        restart: 'Начать заново',
        loading: 'Сжатие…',
        playing: 'Воспроизведение…',
        nothingNew: 'Нечего читать',
        configurePrompt: 'Настройте TTS в Настройках',
    },
