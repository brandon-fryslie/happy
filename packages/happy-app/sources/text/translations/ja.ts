...
export const ja: TranslationStructure = {
    ...
    settings: {
        ...
        tts: 'セッション読み上げ',
        ttsSubtitle: 'エージェントのメッセージを要約して読み上げます',
        featuresTitle: '機能',
    ...
    settingsTts: {
        ...
        enabledTitle: 'セッション読み上げ',
        enabledFooter: 'お使いのLLMでエージェントのメッセージを要約し、ElevenLabsで読み上げます。すべてはお使いのデバイス内に留まり、キーやメッセージのテキストがHappyのサーバーに送信されることはありません。',
        enableLabel: '有効化',
        enableSubtitleOn: 'セッションに読み上げ操作バーを表示',
        enableSubtitleOff: 'セッションでは非表示',
        autoSpeakTitle: '自動読み上げ',
        autoSpeakFooter: 'ハンズフリーでは、アプリがバックグラウンドにあるときや画面がロックされているときも読み上げを続け、返信ごとに間を空けずにすべて読み上げます。',
        autoSpeakOffLabel: 'オフ',
        autoSpeakOffSubtitle: '再生をタップしたときのみ読み上げます',
        autoSpeakForegroundLabel: 'アプリ表示中',
        autoSpeakForegroundSubtitle: 'アプリを開いている間、新しいエージェントのメッセージを読み上げます（返信の間に30秒の間隔）',
        autoSpeakHandsFreeLabel: 'ハンズフリー',
        autoSpeakHandsFreeSubtitle: '画面がオフでもアプリがバックグラウンドでも読み上げを続けます（返信の間に間隔なし）',
        providerTitle: '音声プロバイダー',
        providerFooter: '要約を音声に変換するサービスです。プロバイダーごとに音声とキーを個別に保存するため、切り替えても両方の設定が保持されます。',
        providerElevenLabsLabel: 'ElevenLabs',
        providerElevenLabsSubtitle: '最も自然な音声。ElevenLabsの別アカウントが必要です。',
        providerOpenAiLabel: 'OpenAI',
        providerOpenAiSubtitle: '要約用のものを含め、OpenAIのキーを再利用します',
        openAiTitle: 'OpenAI音声合成',
        openAiFooter: '音声プロバイダーとしてOpenAIを選択したときに使用します。未設定の場合、キーは下の要約用キーを使用します。',
        openAiVoiceLabel: '音声',
        openAiVoiceSubtitleNotSet: 'デフォルトの音声 (Alloy)',
        openAiVoicePrompt: 'OpenAIの音声名を入力してください: alloy、echo、fable、onyx、nova、shimmer。空欄の場合はデフォルトを使用します。',
        openAiVoicePlaceholder: 'alloy',
        openAiKeyLabel: 'API Key',
        openAiKeySubtitleNotSet: '未設定の場合は要約用のキーを使用',
        openAiKeySubtitleSet: '設定済み (非表示)',
        openAiKeyPrompt: 'OpenAIのAPI keyを入力してください。空欄の場合は要約用のキーを再利用します。',
        openAiKeyPlaceholder: 'sk-…',
        elevenLabsTitle: 'ElevenLabs',
        elevenLabsFooter: '音声合成。TTS専用のキーが設定されている場合は、BYOの音声キーよりも優先されます。',
        voiceIdLabel: 'Voice ID',
        voiceIdSubtitleNotSet: 'デフォルトの音声 (Rachel)',
        voiceIdPrompt: 'ElevenLabsのvoice IDを入力してください。空欄の場合はデフォルトを使用します。',
        voiceIdPlaceholder: '21m00Tcm4TlvDq8ikWAM',
        elevenLabsKeyLabel: 'API Key',
        elevenLabsKeySubtitleNotSet: '未設定の場合はBYOの音声キーを使用',
        elevenLabsKeySubtitleSet: '設定済み (非表示)',
        elevenLabsKeyPrompt: 'ElevenLabsのAPI keyを入力してください。空欄の場合はBYOの音声キーを使用します。',
        elevenLabsKeyPlaceholder: 'sk_…',
        llmTitle: '要約 (OpenAI互換)',
        llmFooter: 'OpenAI互換のエンドポイントなら何でも: OpenAI、OpenRouter、Ollama、vLLMなど。ローカルのOllamaには、TailscaleまたはLANのホスト名 (例: http://ollama.local:11434/v1) を使用してください — localhostは電話からは機能しません。',
        llmBaseUrlLabel: 'ベースURL',
        llmBaseUrlSubtitleNotSet: '未設定',
        llmBaseUrlPrompt: 'OpenAI互換のベースURLを入力してください。',
        llmBaseUrlPlaceholder: 'https://api.openai.com/v1',
        llmKeyLabel: 'API Key',
        llmKeySubtitleNotSet: '未設定 (ローカルのOllamaの場合は空欄のまま)',
        llmKeySubtitleSet: '設定済み (非表示)',
        llmKeyPrompt: 'LLMのAPI keyを入力してください。ローカルのOllamaでは空欄でも構いません。',
        llmKeyPlaceholder: 'sk-…',
        llmModelLabel: 'モデル',
        llmModelSubtitleNotSet: '未設定',
        llmModelPrompt: 'モデル名を入力してください。',
        llmModelPlaceholder: 'gpt-4o-mini',
        privacyTitle: 'プライバシー',
        privacyText: 'API keysはこの端末にローカルで保存されます。メッセージのテキストは、設定されたLLMおよびElevenLabsのエンドポイントへ直接送信され、Happyのサーバーには送られません。',
    },
    ...
    sessionTts: {
        ...
        stop: '停止',
        continueOption: '続きから',
        fromLastUser: '最後のメッセージから',
        restart: '最初から再開',
        loading: '要約中…',
        playing: '読み上げ中…',
        nothingNew: '新しい読み上げはありません',
        configurePrompt: '設定でTTSを構成してください',
    },
